...
export const BrochureSampleV2: BrochureSchema = {
  ...
  page1: {
    header: `Verifiable Credentials for Cross-Border Trade and Other Use Cases: Proposed Contributions by Singapore
        Infocomm Media Development Authority and Singapore Government Technology Agency`,
    contents: [
      {
      ...
      },
      {
        ...
        body: `To address these inefficiencies, we have developed TradeTrust. TradeTrust, which is built upon
        OpenAttestation framework, comprises globally accepted standards and frameworks that allow governments
        and businesses to issue, verify and effect title transfer of electronic documents across different
        digital platforms seamlessly. TradeTrust can help to protect the trustworthiness of digital documents
        using cryptography and decentralised identifier (DID) for the verification of their authenticities and
        their provenances.`,
      },
      {
        subheader: `What documents can TradeTrust support in their digitalisation`,
        body: `As a framework, TradeTrust is designed and developed to support the digitalization of two categories of
        documents used in Cross-Border Trade and Trade Financing processes. The first category is known as the
        Verifiable Document where the integrity or the trustworthiness of the document can be verified. Examples
        of such documents include the Certificates of Origin, Commercial Invoices and Packing Lists etc. The
        second category is Transferable document (or document of title) where it confers title ownership to an
        asset and the ownership can be transferred from one party to another. Examples of such documents include
        the negotiable Bill of Lading, Bill of Exchange, Warehouse Receipts and Promissory Notes etc. An
        electronic Bill of Lading (eBL) that is issued using the TradeTrust framework can be transacted by
        various commercial stakeholders in a fully decentralised manner, eliminating the need for all parties to
        be on a common digital platform.`,
      },
      {
        subheader: `How are Selective Redaction and Decentralised Renderer functions used in TradeTrust and why are they useful`,
        bodyAsList: [
          `While TradeTrust facilitates the trusted exchange of digital documents across the different parties in
        cross-border trade processes, it is important to ensure that the holder of these documents have control
        over what information can be shared with other users. To support such flexibilities, TradeTrust adopts
        the `,
          ` method and allows the holder of the digital trade document to redact information
        that are commercially sensitive (such as suppliers' or products' pricing) before sharing it with the
        next receiving party. TradeTrust also supports the concept of `,
          ` of digital
        documents in order to allow the document issuers flexibility in designing the rendering functions
        according to their preference. The decentralised approach also helps to mitigate the risks of single
        point of failure (SPOF) when all documents rendering functions are consolidated under a common hardware
        infrastructure.`,
        ],
    ...
    contents: [
      {
        ...
        body: `As the world moves away from paper documents and towards digital documents, it is important to ensure that
      digital documents are resistant to tampering and trust is preserved. OpenAttestation is an open-source
      framework developed by the Singapore Government Technology Agency to simplify the endorsement and verification
      of digital documents. In production since 2018, OpenAttestation has been implemented in various areas such as
      academic credentials, healthcare records, and cross-border trade.`,
      },
      {
        ...
        listItems: [
          {
            ...
            description: `OpenAttestation ensures that the content of the document has not been modified since the document’s creation,
          with the exception of data omitted using the built-in selective redaction mechanism.`,
          },
          {
          ...
          },
          {
            ...
            description: `OpenAttestation checks and returns the identity of the issuer primarily through the Domain Name System (DNS).
          It verifies that the issuing party in the document has provided some sort of proof that it is the same party
          as claimed by, for example, proving ownership over a domain by the ability to create a DNS record.`,
          },
        ],
      },
      {
    ...
    contents: [
      {
        ...
        body: `To improve interoperability and adoption, our team proposes the following contributions to the W3C Verifiable
        ...
        listItems: [
          {
            ...
            descriptionAsList: [
              `Selective redaction is a desirable property of verifiable credentials. OpenAttestation implements a lossy
            progressive selective redaction scheme, which allows it to be used in applications where a verifiable credential
            is passed along a chain of multiple verifiers and where privacy and data minimisation techniques are desired
            and/or required.`,
              `This proof method can be understood as a “hash of hashes” in which each property of the
            credential is salted and hashed. The output hashes are sorted, stored in an array, and hashed again to derive
            the target hash, which is akin to a digital fingerprint of the credential. To redact a property in the
            OpenAttestation Verifiable Credential, the obfuscated data is replaced by its corresponding derivated hash.
            Obfuscation does not invalidate the proof property as the target hash is computed over all hashed key-value
            pairs in the credential, including obfuscated property hashes.`,
              `The Verifiable Credentials Data Model is proof
            format agnostic and does not normatively require any particular digital proof or signature format. As the
            OpenAttestationMerkleProof2018 Signature Method is being actively utilised by OpenAttestation implementations,
            our team believes that documenting the OpenAttestationMerkleProof2018 Signature Method as part of the Verifiable
            Credentials Data Model would be beneficial to implementers.`,
            ],
          },
          {
          ...
          },
          {
            ...
            description: `OpenAttestation enables paper-friendly verifiable credentials that are suitable for use cases with uneven
            technological maturity, such as long cross-border supply chains. OpenAttestation verifiable credentials
            can be issued with a QR code which, when scanned with QR code reader such as a mobile phone, will direct
            the viewer to the OpenAttestation document viewer and load the OpenAttestation verifiable credential using
            the decentralised renderer.`,
          },
        ],
      },
    ],
